    ...
    def forward(
        ...
        for idx, layer in enumerate(
            ...
            if idx + 1 in self.aux_hidden_state_layers:
                ...
                # HERETIC PATCH: collapse the hyper-connection copies with the
                # carried pre-mix instead of with a plain mean.
                #
                # This line used to be `aux_recon.mean(dim=1)` -- a uniform
                # average over the hc_mult copies. The residual stream the next
                # layer actually consumes is the mix-weighted collapse
                # `hc_collapse_triton(aux_recon, pre_mix)`, the same call this
                # module already makes at the model's final collapse.
                #
                # The two agree only if the learned mixes are uniform, and they
                # are not: across all 40 layers std/|mean| is 1.93 for
                # hc_attn_base and 0.67 for hc_attn_scale, and at layer 0 the
                # per-copy biases alone span 39 logit units (-25.49 .. +13.44).
                # Through a softmax a spread that wide makes the real collapse
                # behave closer to selecting one copy than to averaging four.
                #
                # A direction measured on the mean is therefore close to
                # orthogonal to the direction the model reads. That is exactly
                # the observed failure: refusal directions that separate harmful
                # from harmless prompts perfectly, an ablation the engine
                # provably applies, and no change in refusal behaviour.
                #
                # `pre_mix` here is the mix the current layer returned for the
                # NEXT layer's attention, which is what that layer's
                # mhc_pre_delayed_tilelang will collapse with -- verified against
                # the call at line 337.
